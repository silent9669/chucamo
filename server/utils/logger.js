...
const currentLogLevel = process.env.LOG_LEVEL || (isProduction ? 'WARN' : 'DEBUG');

const shouldLog = (level) => {
  return LOG_LEVELS[level] <= LOG_LEVELS[currentLogLevel];
};

// Rate limiting for logs to prevent Railway rate limit issues
let logCount = 0;
let lastResetTime = Date.now();
const LOG_RATE_LIMIT = 100; // Max logs per second
